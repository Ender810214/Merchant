...
uses
  ...
  cxLookAndFeels, cxLookAndFeelPainters, cxEdit, cxLabel, cxDBLabel,
  Vcl.StdCtrls,
  ...
  dxSkinXmas2008Blue, dxScrollbarAnnotations, dxSkinWXI;
...
uses main, sel_cliente_fac, sel_comp, prod_fact, sel_contrato_fac, transfact,
  facturadores, sel_ordend_factura, change_number;
{$R *.dfm}
...
begin
  ...
  begin
    ...
    fprod_fact.QProd_Fact.ParamByName('factura').Value :=
      UniQuery2facturaid.Value;
    fprod_fact.QProd_Fact.ParamByName('fecha').Value := UniQuery2fecha.Value;
...
begin
  if MessageDlg
    ('Esta operación devolverá la factura seleccionada al estado EN PROCESO y retornará los productos facturados al AlMACEN, Desea Continuar?',
...
begin
  ...
  Qcontratos1.Open;
  QCompradores.Open;
  UniQuery2.ParamByName('in_fecha1').Value := QPeriododesde.Value;
  UniQuery2.ParamByName('in_fecha2').Value := QPeriodohasta.Value;
  UniQuery2.ExecSQL;
  UniQuery2.Open;
end;
